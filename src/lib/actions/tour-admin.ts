...
export async function saveTour(input: TourAdminInput): Promise<TourActionState> {
  ...
  const existing = data.id ? await prisma.tour.findUnique({ where: { id: data.id }, select: { id: true, slug: true, heroImagePublicId: true, cardImagePublicId: true, itineraryDays: { select: { id: true, imagePublicId: true } }, images: { select: { id: true, cloudinaryPublicId: true } } } }) : null;
  ...
  const existingDayAssets = new Map(existing?.itineraryDays.map((day) => [day.id, day.imagePublicId]) ?? []);
  const existingGalleryAssets = new Map(existing?.images.map((image) => [image.id, image.cloudinaryPublicId]) ?? []);
  const submittedAssets = [
    { publicId: data.heroImagePublicId, secureUrl: data.heroImagePath, existingPublicId: existing?.heroImagePublicId },
    { publicId: data.cardImagePublicId, secureUrl: data.cardImagePath, existingPublicId: existing?.cardImagePublicId },
    ...data.itineraryDays.map((day) => ({ publicId: day.imagePublicId, secureUrl: day.imagePath, existingPublicId: day.id ? existingDayAssets.get(day.id) : null })),
    ...data.images.map((image) => ({ publicId: image.cloudinaryPublicId, secureUrl: image.imagePath, existingPublicId: image.id ? existingGalleryAssets.get(image.id) : null })),
  ];
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      await tx.tour.upsert({
        where: { id: tourId },
        update: { slug: data.slug, packageType: data.packageType, durationDays: data.durationDays, heroImagePath: data.heroImagePath, heroImagePublicId: clean(data.heroImagePublicId), cardImagePath: clean(data.cardImagePath), cardImagePublicId: clean(data.cardImagePublicId), featured: data.featured, status: data.status, displayOrder: data.displayOrder, needsClientConfirmation: data.needsClientConfirmation },
        create: { id: tourId, slug: data.slug, packageType: data.packageType, durationDays: data.durationDays, heroImagePath: data.heroImagePath, heroImagePublicId: clean(data.heroImagePublicId), cardImagePath: clean(data.cardImagePath), cardImagePublicId: clean(data.cardImagePublicId), featured: data.featured, status: data.status, displayOrder: data.displayOrder, needsClientConfirmation: data.needsClientConfirmation },
      });
      ...
      for (const day of data.itineraryDays) {
        ...
        await tx.tourItineraryDay.create({ data: { id: dayId, tourId, dayNumber: day.dayNumber, displayOrder: day.displayOrder, imagePath: clean(day.imagePath), imagePublicId: clean(day.imagePublicId), status: day.status, needsClientConfirmation: day.needsClientConfirmation, translations: { create: (["en", "ar"] as const).map((locale) => ({ id: `${dayId}-${locale}`, locale, title: day[locale].title, description: day[locale].description, overnight: clean(day[locale].overnight), location: clean(day[locale].location), imageAlt: clean(day[locale].imageAlt) })) } } });
      }

      await tx.tourImage.deleteMany({ where: { tourId } });
      for (const image of data.images) {
        const imageId = id(image.id, `${tourId}-image`);
        await tx.tourImage.create({ data: { id: imageId, tourId, imagePath: image.imagePath, cloudinaryPublicId: clean(image.cloudinaryPublicId), displayOrder: image.displayOrder, status: image.status, translations: { create: (["en", "ar"] as const).map((locale) => ({ id: `${imageId}-${locale}`, locale, altText: image[locale].altText, title: clean(image[locale].title) })) } } });
      }
    ...
    for (const publicId of newlyClaimed) await deleteOwnedCloudinaryAsset(publicId);
    return { ok: false, error: "The tour could not be saved. No partial changes were applied." };
  }
  const previousPublicIds = [existing?.heroImagePublicId, existing?.cardImagePublicId, ...existingDayAssets.values(), ...existingGalleryAssets.values()].filter((value): value is string => Boolean(value));
  const currentPublicIds = new Set(submittedAssets.map((asset) => asset.publicId).filter((value): value is string => Boolean(value)));
  for (const publicId of previousPublicIds) if (!currentPublicIds.has(publicId)) await deleteOwnedCloudinaryAsset(publicId);
...
export async function deleteOrArchiveTour(formData: FormData) {
  ...
  const tour = await prisma.tour.findUnique({ where: { id: tourId }, select: { slug: true, heroImagePublicId: true, cardImagePublicId: true, itineraryDays: { select: { imagePublicId: true } }, images: { select: { cloudinaryPublicId: true } }, _count: { select: { bookingRequests: true } } } });
  ...
  for (const publicId of [tour.heroImagePublicId, tour.cardImagePublicId, ...tour.itineraryDays.map((day) => day.imagePublicId), ...tour.images.map((image) => image.cloudinaryPublicId)]) await deleteOwnedCloudinaryAsset(publicId);
  revalidateTourPaths(tour.slug);
