...
export function createContactRequestSchema(locale: Locale, acceptedValues: readonly string[] = contactEnquiryValues) {
  const ar = locale === "ar";
  ...
  return z.object({
    ...
    enquiryType: z.string().trim().max(120).refine(value => accepted.has(value), ar ? "اختر نوع استفسار صالحًا." : "Select a valid enquiry type."),
