...
import { getGallerySeo, getPublishedGalleryContent } from "@/lib/gallery/gallery-repository";

export async function generateMetadata({ params }: PageProps<"/[locale]/gallery">): Promise<Metadata> { const { locale } = await params; if (!isLocale(locale)) notFound(); const data = await getGallerySeo(locale); if (!data) notFound(); return { title: data.title, description: data.description, alternates: { languages: languageAlternates("/gallery") }, openGraph: { title: data.title, description: data.description, type: "website", url: `/${locale}/gallery`, images: data.imagePath ? [{ url: data.imagePath }] : undefined } }; }

export default async function GalleryPage({ params }: PageProps<"/[locale]/gallery">) {
  const { locale } = await params; if (!isLocale(locale)) notFound();
  const [managed, dict] = await Promise.all([getPublishedGalleryContent(locale), getDictionary(locale)]);
