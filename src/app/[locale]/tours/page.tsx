...
export default async function ToursPage({params}:PageProps<"/[locale]/tours">){const{locale}=await params;if(!isLocale(locale))notFound();const ui=getTourUI(locale);const tours=await getPublishedTours(locale);return <main><ToursPageHero content={{eyebrow:ui.listing.eyebrow,heading:ui.listing.heading,description:ui.listing.description,placeholderNotice:ui.listing.placeholderNotice}}/><Section className="bg-soft-sand"><Container><ToursGrid tours={tours} locale={locale} labels={{view:ui.labels.viewTour,from:ui.labels.from,perPerson:ui.labels.perPerson,contactPricing:ui.labels.contactPricing}}/></Container></Section><Section className="bg-white pt-0"><ToursPageCTA content={{heading:ui.listing.ctaHeading,primaryAction:{label:ui.listing.plan,href:localizedHref(locale,"/booking")},whatsappAction:{label:ui.listing.whatsapp,href:siteSettingsPlaceholder.contact.whatsappUrl}}}/></Section></main>}
