...
export const emptyTour: TourAdminInput = { slug: "", packageType: "group", durationDays: 8, heroImagePath: "", cardImagePath: "", featured: false, status: "draft", displayOrder: 0, needsClientConfirmation: false, en: { ...defaultTranslation }, ar: { ...defaultTranslation }, pricingTiers: [], included: [], excluded: [], requiredExtras: [], itineraryDays: [], images: [] };
  ...
  return <form onSubmit={submit} className="space-y-7" noValidate>
    ...
    <fieldset className={panel}><legend className="px-2 font-display text-2xl font-semibold">Shared business data</legend>
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <label className="text-sm font-semibold sm:col-span-2">Hero image path<input {...form.register("heroImagePath")} className={`${input} mt-2`} /></label>
        <label className="text-sm font-semibold">Card image path (optional)<input {...form.register("cardImagePath")} className={`${input} mt-2`} /></label>
    ...
    <fieldset className={panel}><legend className="px-2 font-display text-2xl font-semibold">Itinerary</legend>
      <button type="button" onClick={() => itinerary.append({ dayNumber: itinerary.fields.length + 1, displayOrder: itinerary.fields.length + 1, imagePath: "", status: "published", needsClientConfirmation: false, en: { title: "", description: "", overnight: "", location: "", imageAlt: "" }, ar: { title: "", description: "", overnight: "", location: "", imageAlt: "" } })} className="mb-5 rounded-full border px-4 py-2 text-sm font-semibold">Add day</button>
      <div className="space-y-5">{itinerary.fields.map((field, index) => <div key={field.id} className="rounded-xl border p-4"><input type="hidden" {...form.register(`itineraryDays.${index}.id`)} /><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><label className="text-sm">Day<input type="number" {...form.register(`itineraryDays.${index}.dayNumber`, { valueAsNumber: true })} className={`${input} mt-1`} /></label><label className="text-sm">Order<input type="number" {...form.register(`itineraryDays.${index}.displayOrder`, { valueAsNumber: true })} className={`${input} mt-1`} /></label><label className="text-sm">Image path<input {...form.register(`itineraryDays.${index}.imagePath`)} className={`${input} mt-1`} /></label><StatusSelect registration={form.register(`itineraryDays.${index}.status`)} /></div>
        <div className="mt-4 grid gap-4 lg:grid-cols-2">{(["en", "ar"] as const).map((locale) => <div key={locale} dir={locale === "ar" ? "rtl" : "ltr"} className="space-y-3 rounded-xl bg-soft-sand/60 p-4"><p className="font-semibold">{locale === "en" ? "English" : "Arabic"}</p><input placeholder="Title" {...form.register(`itineraryDays.${index}.${locale}.title`)} className={input} /><textarea placeholder="Description" {...form.register(`itineraryDays.${index}.${locale}.description`)} className={textarea} /><input placeholder="Overnight" {...form.register(`itineraryDays.${index}.${locale}.overnight`)} className={input} /><input placeholder="Location" {...form.register(`itineraryDays.${index}.${locale}.location`)} className={input} /><input placeholder="Image alt text" {...form.register(`itineraryDays.${index}.${locale}.imageAlt`)} className={input} /></div>)}</div><button type="button" onClick={() => itinerary.remove(index)} className="mt-3 text-sm font-semibold text-red-700 underline">Remove day</button></div>)}</div>
    </fieldset>

    <fieldset className={panel}><legend className="px-2 font-display text-2xl font-semibold">Gallery image records</legend><p className="mb-4 text-sm text-charcoal/65">Records only—image uploading is not implemented.</p>
      <button type="button" onClick={() => images.append({ imagePath: "", displayOrder: images.fields.length + 1, status: "published", en: { altText: "", title: "" }, ar: { altText: "", title: "" } })} className="mb-5 rounded-full border px-4 py-2 text-sm font-semibold">Add image record</button>
      <div className="space-y-4">{images.fields.map((field, index) => <div key={field.id} className="grid gap-3 rounded-xl border p-4 sm:grid-cols-2 lg:grid-cols-4"><input type="hidden" {...form.register(`images.${index}.id`)} /><input placeholder="Image path" {...form.register(`images.${index}.imagePath`)} className={input} /><input type="number" aria-label="Display order" {...form.register(`images.${index}.displayOrder`, { valueAsNumber: true })} className={input} /><StatusSelect registration={form.register(`images.${index}.status`)} /><input placeholder="English alt text" {...form.register(`images.${index}.en.altText`)} className={input} /><input dir="rtl" placeholder="Arabic alt text" {...form.register(`images.${index}.ar.altText`)} className={input} /><input placeholder="English title" {...form.register(`images.${index}.en.title`)} className={input} /><input dir="rtl" placeholder="Arabic title" {...form.register(`images.${index}.ar.title`)} className={input} /><button type="button" onClick={() => images.remove(index)} className="text-sm font-semibold text-red-700 underline">Remove image</button></div>)}</div>
    </fieldset>
